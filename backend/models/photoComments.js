...
class Comment{
    ...
    static async createCommentsForPost({comment, user, postsId}){
        ...
        const existingComment = await Comment.fetchCommentForPostByUser({ user , postsId})
        if(existingComment){
            throw BadRequestError(`Users are not allowed to leave multiple comments for a single post`)
        }
        const results = await db.query(
            `
